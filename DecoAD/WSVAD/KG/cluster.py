...
def scene_cluster(auc_1 = 0.,flag1 = 0):
    ...
    if not os.path.exists(file_name):
        ...
        if args.dataset == 'UBnormal':
            for i in range(29):
                data = torch.load(f"{args.data_dir}{args.dataset}_scene_feature/scene{i + 1}_features.pth", map_location='cpu')
                dataset_na += data
            ...
            for i in range(len(scene_arr)):
                data = torch.load(f"{args.data_dir}{args.dataset}_scene_feature/scene{scene_arr[i+1]}_features.pth", map_location='cpu')
                dataset_na += data
        else:
