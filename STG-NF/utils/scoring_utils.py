...
def get_dataset_scores(scores, metadata, args=None):
    ...
    if args.dataset == 'UBnormal':
        ...
        per_frame_scores_root = os.path.join(os.path.dirname(os.path.dirname(pose_segs_root)), 'gt/')
        if not os.path.exists(per_frame_scores_root):
            per_frame_scores_root = os.path.join(args.data_dir, args.dataset, 'gt/')
        
        raw_clip_list = sorted(fn for fn in os.listdir(pose_segs_root) if fn.endswith('.json'))
        clip_list = []
        for fn in raw_clip_list:
            npy_name = fn.replace("alphapose_tracked_person.json", "tracks.npy")
            txt_name = fn.replace("alphapose_tracked_person.json", "tracks.txt")
            if os.path.exists(os.path.join(per_frame_scores_root, npy_name)):
                clip_list.append(npy_name)
            else:
                clip_list.append(txt_name)
    else:
        dataset_dir_name = "ShanghaiTech" if "ShanghaiTech" in args.dataset else args.dataset
        per_frame_scores_root = os.path.join(args.data_dir, dataset_dir_name, 'gt/test_frame_mask/')
        if not os.path.exists(per_frame_scores_root):
...
def get_clip_score(scores, clip, metadata_np, metadata, per_frame_scores_root, args):
    ...
    if os.path.exists(clip_res_fn):
        ...
        if len(clip_metadata) > 0:
            max_frame = max([int(arr[3]) for arr in clip_metadata])
        num_frames = max_frame + int(args.seg_len)
        clip_gt = np.zeros(num_frames)  # Dummy ground truth
